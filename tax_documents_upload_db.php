...
/*	$arfields = array(
	...
	{
		curl_setopt_array($curl, array(
		  CURLOPT_URL => API_URL."TaxApplication/SaveTaxApplicationDocument",
		  CURLOPT_RETURNTRANSFER => true,
		  CURLOPT_ENCODING => "",
		  CURLOPT_MAXREDIRS => 10,
		  CURLOPT_TIMEOUT => 0,
    ...
    //if( strtolower($res['status']) == 'success' )
    {
		...
		$db->location(SITEURL.'tax-documents/');
		exit;
	}
	...
	else
	{
		...
		if( $mode == 'add' )
			$db->location(SITEURL.'tax-documents-upload/');
		else
			$db->location(SITEURL.'tax-documents-upload/'.$id.'/');
		exit;
	}
?>